#!/usr/bin/env python
"""
Batch evaluation script for GRPO checkpoints.
Evaluates multiple checkpoints and gathers results into a summary table.
"""
import os
import sys
import torch
...
def evaluate_checkpoint(checkpoint_dir, step, device, autocast_ctx, 
    ...
    # Load model
    print0(f"\n{'='*60}")
    print0(f"Loading checkpoint step {step} from {checkpoint_dir}")
    print0(f"{'='*60}")
    ...
    if ddp_rank == 0:
        pbar = tqdm(total=num_eval, desc=f"Step {step} Pass@{pass_at_k}", unit="sample")
...
def main():
    # Configuration
    checkpoint_base = "/home/jl77863/.cache/nanochat/chatrl_checkpoints_v5"
    model_tag = "d32"
    checkpoint_dir = os.path.join(checkpoint_base, model_tag)
    
    # Checkpoints to evaluate (v5: 100, 200, 300, 400, 466)
    steps_to_eval = [100, 200, 300, 400, 466]
    
    # Eval settings
    num_samples = 1000  # Full evaluation per checkpoint
    ...
    print0(f"\n{'#'*60}")
    print0(f"BATCH CHECKPOINT EVALUATION")
    print0(f"{'#'*60}")
    print0(f"Checkpoint dir: {checkpoint_dir}")
    print0(f"Steps to evaluate: {steps_to_eval}")
    print0(f"Samples per checkpoint: {num_samples}")
    ...
    for step in steps_to_eval:
        ...
        if result:
            ...
            if ddp_rank == 0:
                print0(f"\n✅ Step {step}: Pass@{pass_at_k} = {result['accuracy']:.4f} ({result['correct']}/{result['total']})")
    ...
    if ddp_rank == 0:
        ...
        print0(f"{'='*60}")
        print0(f"{'Step':>8} | {'Pass@1':>10} | {'Correct':>8} | {'Total':>8}")
        print0("-" * 45)
        for r in results:
            print0(f"{r['step']:>8} | {r['accuracy']:>10.4f} | {r['correct']:>8} | {r['total']:>8}")
        print0(f"{'='*60}\n")
        
        # Find best checkpoint
        if results:
            best = max(results, key=lambda x: x['accuracy'])
            print0(f"🏆 Best checkpoint: Step {best['step']} with Pass@1 = {best['accuracy']:.4f}")
        
        # Save results to file
        output_dir = "eval_results"
        os.makedirs(output_dir, exist_ok=True)
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        output_path = os.path.join(output_dir, f"batch_eval_v5_{timestamp}.json")
        ...
        summary = {
            ...
            "temperature": temperature,
            "results": results,
            "best_step": best['step'] if results else None,
